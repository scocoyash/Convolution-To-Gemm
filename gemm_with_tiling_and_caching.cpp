...
inline void gemm_tiling_and_caching
(
    const int M,                       // H
    const int N,                       // W
    const int K,                       // C
...
int main() {
  ...
  gemm_tiling_and_caching(H, W, C, x, y, out_tiling_and_caching);
  gettimeofday(&end, NULL);
  long seconds  = end.tv_sec  - start.tv_sec;
  long useconds = end.tv_usec - start.tv_usec;  
  float mtime = ((seconds) * 1000 + useconds/1000.0);
  printf("Gemm with Tiling + Caching Elapsed time: %f milliseconds GFlops: %f\n", mtime, ((float) 2*H*W*C)/(mtime*1e6));
