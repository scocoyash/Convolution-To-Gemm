...
float out_naive[H * W]  __attribute__((aligned(256)));
...
)
{
  ...
  for (int m=0; m<M; ++m)
  {
    for (int n=0; n<N; ++n)
    {
      for (int k=0; k<K; ++k)
      {
        size_t A_idx = m*K + k, B_idx = n + k*N;
